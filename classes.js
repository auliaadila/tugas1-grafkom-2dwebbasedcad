    ...
    */
    constructor(gl, points, color, GL_SHAPE) {
        if (this.constructor === Shape) {
            throw new Error('ERROR: Tidak bisa membuat objek dari kelas abstrak "Shape"')
        }
...
class Tool {
    ...
    constructor(canvas, gl, current) {
        if (this.constructor === Tool) {
            throw new Error('ERROR: Tidak bisa membuat objek dari kelas abstrak "Tool"')
        }
    ...
    drawCanvas() {
        this.gl.clearColor(this.current.canvasColor.red, this.current.canvasColor.green, this.current.canvasColor.blue, 1)
     ...
     */
    getCursorPosition(e) {
        return new Point((((e.offsetX / this.canvas.clientWidth) * 2) - 1), (((1 - (e.offsetY / this.canvas.clientHeight)) * 2) - 1))
    }
}
...
class SelectionTool extends Tool {
    ...
    setActiveShapeColor() {
        if ((this.activeShapeIndex > -1) && (this.activeShapeIndex < this.current.shapes.length)) {
            this.current.shapes[this.activeShapeIndex].color.copyColor(this.current.shapeColor)
        }
    }
    ...
    */
    isCursorInside(shape, cursorPosition) {
        ...
        if ((cursorPosition.x >= minX) && (cursorPosition.x <= maxX) && (cursorPosition.y >= minY) && (cursorPosition.y <= maxY)) {
            return true
        } else {
            return false
        }
    ...
    clickListener(e) {
        ...
        if ((temp > -1) && (temp < this.current.shapes.length) && (temp === this.activeShapeIndex)) {
     ...
     */
    mouseDownListener(e) {
        if ((this.dragging) && (this.activeShapeIndex > -1) && (this.activeShapeIndex < this.current.shapes.length) && (this.tempPoints.length === this.current.shapes[this.activeShapeIndex].points.length)) {
            let dragPosition = this.getCursorPosition(e)
            for (let i = 0; i < this.tempPoints.length; i += 1) {
                this.current.shapes[this.activeShapeIndex].points[i].x = this.tempPoints[i].x + (dragPosition.x - this.clickPosition.x)
                this.current.shapes[this.activeShapeIndex].points[i].y = this.tempPoints[i].y + (dragPosition.y - this.clickPosition.y)
            }
            this.drawCanvas()
        }
    }
...
class LineTool extends Tool {
    ...
    clickListener(e) {
        if (this.isDrawing) {
            ...
            this.line = new Line(this.gl, [point, new Point()], this.current.shapeColor)
            this.isDrawing = true
        }
    }
...
class PolygonTool extends Tool {
    ...
    clickListener(e) {
        if (this.isDrawing) {
            ...
            let point = this.getCursorPosition(e)
            this.polygon = new Polygon(this.gl, [point, new Point()], this.current.shapeColor)
            this.isDrawing = true
        }
    }
    ...
    mouseMoveListener(e) {
        if (this.isDrawing) {
            this.drawCanvas()
            this.polygon.points[this.polygon.points.length - 1].copyPoint(this.getCursorPosition(e))
            this.polygon.draw()
        }
    }
